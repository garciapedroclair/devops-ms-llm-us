...
@app.get("/tasks_time_by_skill")
def tasks_time_by_skill(skill: str = Query(..., description="Skill to filter (e.g., agile_methods)")):
    numeric_skills = ["sw_project_mgmt", "requirements", "agile_methods", "llm_usage"]
...
@app.get("/tasks_grade_by_skill")
def tasks_grade_by_skill(skill: str = Query(..., description="Skill to filter (e.g., agile_methods)")):
    numeric_skills = ["sw_project_mgmt", "requirements", "agile_methods", "llm_usage"]
    if skill not in numeric_skills:
        return {"error": f"Skill '{skill}' is not valid. Choose one of {numeric_skills}"}
